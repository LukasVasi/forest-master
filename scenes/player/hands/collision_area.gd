...
func _physics_process(_delta: float) -> void:
	...
	for preserved_exception : Node in _preserved_collision_exceptions:
		if is_instance_valid(preserved_exception) and preserved_exception not in overlapping_bodies:
...
func preserve_collision_exception_with(body: Node) -> void:
	if is_instance_valid(body) and get_overlapping_bodies().has(body):
