...
# Remove an event
func remove(event_key: Variant, trackers: Array = ALL_TRACKERS) -> void:
	if not event_key:
		push_error("Event key is invalid!")
		return
...
func clear(trackers: Array = ALL_TRACKERS) -> void:
	# Substitube the shorthand for all trackers with the real thing
	if trackers == ALL_TRACKERS:
		trackers = _queues.keys()
	...
	for tracker in trackers:
		...
		# Remove all events and their remaining time from the respective queues
		_queues[tracker].events.clear()
		_queues[tracker].time_remaining.clear()
