...
class_name PhysicalFunctionPickupV2
extends Node3D
...
## Controller that this pickup is under
@onready var _controller := XRHelpers.get_xr_controller(self)

## Physical hand that this pickup is under
@onready var hand : PhysicalHand = _get_physical_hand()
...
func _get_physical_hand() -> PhysicalHand:
	var node: Node = get_parent()
	
	while node:
		if (node is PhysicalHand):
			break
